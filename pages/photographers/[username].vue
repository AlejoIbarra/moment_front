<template>
  <div class="ig-container pt-8 px-4">
    ...
    <div v-else>
      <!-- Profile Header -->
      <header
        class="flex flex-col md:flex-row items-center border-b border-[#dbdbdb] pb-12 mb-10 space-y-8 md:space-y-0 md:space-x-20 px-4 md:px-10">
        <!-- Avatar -->
          ...
          <div
            ...
            <div class="bg-white p-[3px] rounded-full w-full h-full">
              <div
                ...
                <img v-if="photographer.profilePhotoUrl" :src="photographer.profilePhotoUrl" alt="Profile"
                  class="w-full h-full object-cover">
                <span v-else class="text-3xl md:text-5xl font-bold text-indigo-600">{{
                  photographer.username.charAt(0).toUpperCase() }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="flex-1 text-center md:text-left">
          ...
          <div class="flex justify-center md:justify-start space-x-10 mb-6">
            ...
            <span class="text-[#262626] cursor-pointer"><strong class="font-semibold">{{ photographer.followerCount
          ...
          <div>
            ...
            <p class="text-sm text-[#262626] mt-1 whitespace-pre-wrap">{{ photographer.description || 'Fotografo 📸✨' }}
            </p>
          </div>
        </div>
      </header>
      ...
      <!-- Tabs -->
      <div
        class="flex justify-center border-t border-[#dbdbdb] mt-[-1px] uppercase tracking-widest text-[12px] font-semibold text-gray-500 space-x-14">
      ...
      <div v-else class="grid grid-cols-3 gap-1 md:gap-7 mt-8">
        <div v-for="event in events" :key="event.id"
          ...
          <!-- Hover Overlay (Stats) -->
          <div
            class="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center space-x-6 text-white z-20">
          ...
          <div class="absolute bottom-0 left-0 right-0 bg-black/30 text-white p-2 z-10">
            <div class="flex justify-between items-center">
              <span class="text-[10px] md:text-xs font-bold truncate pr-2 uppercase tracking-tight">{{ event.title
                }}</span>
              <span class="text-[9px] md:text-[10px] font-medium whitespace-nowrap opacity-80">{{ event.photoCount || 0
                }} fotos</span>
            </div>
          </div>
        </div>
